...
public class CeratoEntity extends Animal implements IAnimatable {

    private final AnimationFactory factory = GeckoLibUtil.createFactory(this);

    protected CeratoEntity(EntityType<? extends Animal> entityType, Level level) {
...
//        return DeadlyDinosSoundEvents.MED_CHUFF.get();
//    }
        ...
        this.targetSelector.addGoal(6, new NearestAttackableTargetGoal<>(this, LivingEntity.class, 10, true, true, new Predicate<LivingEntity>() {
            ...
            public boolean test(@Nullable LivingEntity livingEntity) {
                if (livingEntity instanceof MahakalaEntity)
                    return false;
                if (livingEntity instanceof CeratoEntity)
                    return false;
                if (livingEntity instanceof CeratoEntity)
                    return false;
    ...
    private <E extends IAnimatable>PlayState predicate(AnimationEvent<E> event) {
        ...
        if (event.isMoving()) {
            if (isAggressive()) {
                event.getController().setAnimation(new AnimationBuilder().addAnimation("ceratosprint", ILoopType.EDefaultLoopTypes.LOOP));

            } else
                event.getController().setAnimation(new AnimationBuilder().addAnimation("ceratowalk", ILoopType.EDefaultLoopTypes.LOOP));

        } else
            event.getController().setAnimation(new AnimationBuilder().addAnimation("ceratoidle", ILoopType.EDefaultLoopTypes.LOOP));
    ...
    private static final EntityDataAccessor<Integer> VARIANT = SynchedEntityData.defineId(CeratoEntity.class, EntityDataSerializers.INT);
