...
public class MajungaEntity extends Animal implements IAnimatable {

    private final AnimationFactory factory = GeckoLibUtil.createFactory(this);

    protected MajungaEntity(EntityType<? extends Animal> entityType, Level level) {
        ...
        this.targetSelector.addGoal(6, new NearestAttackableTargetGoal<>(this, LivingEntity.class, 10, true, true, new Predicate<LivingEntity>() {
            ...
            public boolean test(@Nullable LivingEntity livingEntity) {
                if (livingEntity instanceof MahakalaEntity)
                    return false;
                if (livingEntity instanceof MajungaEntity)
                    return false;
    ...
    private <E extends IAnimatable>PlayState predicate(AnimationEvent<E> event) {
        ...
        if (event.isMoving()) {
            if (isAggressive()) {
                event.getController().setAnimation(new AnimationBuilder().addAnimation("majungasprint", ILoopType.EDefaultLoopTypes.LOOP));

            } else
                event.getController().setAnimation(new AnimationBuilder().addAnimation("majungawalk", ILoopType.EDefaultLoopTypes.LOOP));

        } else
            event.getController().setAnimation(new AnimationBuilder().addAnimation("majungaidle", ILoopType.EDefaultLoopTypes.LOOP));
    ...
    private static final EntityDataAccessor<Integer> VARIANT = SynchedEntityData.defineId(MajungaEntity.class, EntityDataSerializers.INT);
