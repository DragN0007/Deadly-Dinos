...
public class DeinonEntity extends Animal implements IAnimatable {

    private final AnimationFactory factory = GeckoLibUtil.createFactory(this);

    protected DeinonEntity(EntityType<? extends Animal> entityType, Level level) {
    ...
    protected void registerGoals() {
        ...
        this.targetSelector.addGoal(6, new NearestAttackableTargetGoal<>(this, LivingEntity.class, 10, true, true, new Predicate<LivingEntity>() {
            ...
            public boolean test(@Nullable LivingEntity livingEntity) {
                if (livingEntity instanceof AustroEntity)
                    return false;
                if (livingEntity instanceof UtahEntity)
                    return false;
                if (livingEntity instanceof DeinonEntity)
                    return false;
    ...
    private <E extends IAnimatable>PlayState predicate(AnimationEvent<E> event) {
        ...
        if (event.isMoving()) {
            if (isAggressive()) {
                event.getController().setAnimation(new AnimationBuilder().addAnimation("deinonsprint", ILoopType.EDefaultLoopTypes.LOOP));

            } else
                event.getController().setAnimation(new AnimationBuilder().addAnimation("deinonwalk", ILoopType.EDefaultLoopTypes.LOOP));

        } else
            event.getController().setAnimation(new AnimationBuilder().addAnimation("deinonidle", ILoopType.EDefaultLoopTypes.LOOP));
    ...
    private static final EntityDataAccessor<Integer> VARIANT = SynchedEntityData.defineId(DeinonEntity.class, EntityDataSerializers.INT);
