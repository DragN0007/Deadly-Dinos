...
public class AcroEntity extends Animal implements IAnimatable {

    private final AnimationFactory factory = GeckoLibUtil.createFactory(this);

    protected AcroEntity(EntityType<? extends Animal> entityType, Level level) {
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(2, new DinoMeleeGoal(this, 1.8, true));
    ...
    private <E extends IAnimatable>PlayState predicate(AnimationEvent<E> event) {
        ...
        if (event.isMoving()) {
            if (isAggressive()) {
                event.getController().setAnimation(new AnimationBuilder().addAnimation("acrosprint", ILoopType.EDefaultLoopTypes.LOOP));

            } else
                event.getController().setAnimation(new AnimationBuilder().addAnimation("acrowalk", ILoopType.EDefaultLoopTypes.LOOP));

        } else
            event.getController().setAnimation(new AnimationBuilder().addAnimation("acroidle", ILoopType.EDefaultLoopTypes.LOOP));
    ...
    public ResourceLocation getTextureLocation() {
        return AcroModel.Variant.variantFromOrdinal(getVariant()).resourceLocation;
    }


    private static final EntityDataAccessor<Integer> VARIANT = SynchedEntityData.defineId(AcroEntity.class, EntityDataSerializers.INT);
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor levelAccessor, DifficultyInstance difficultyInstance, MobSpawnType mobSpawnType, @Nullable SpawnGroupData spawnGroupData, @Nullable CompoundTag compoundTag) {
        ...
        setVariant(new Random().nextInt(AcroModel.Variant.values().length));
