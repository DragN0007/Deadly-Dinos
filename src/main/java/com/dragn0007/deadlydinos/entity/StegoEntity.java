...
public class StegoEntity extends Animal implements IAnimatable {

    private final AnimationFactory factory = GeckoLibUtil.createFactory(this);

    protected StegoEntity(EntityType<? extends Animal> entityType, Level level) {
    ...
    private <E extends IAnimatable>PlayState predicate(AnimationEvent<E> event) {
        ...
        if (event.isMoving()) {
            if (isAggressive()) {
                event.getController().setAnimation(new AnimationBuilder().addAnimation("stegoattack", ILoopType.EDefaultLoopTypes.LOOP));

            } else
                event.getController().setAnimation(new AnimationBuilder().addAnimation("stegowalk", ILoopType.EDefaultLoopTypes.LOOP));

        } else
            event.getController().setAnimation(new AnimationBuilder().addAnimation("stegoidle", ILoopType.EDefaultLoopTypes.LOOP));
    ...
    private static final EntityDataAccessor<Integer> VARIANT = SynchedEntityData.defineId(StegoEntity.class, EntityDataSerializers.INT);
