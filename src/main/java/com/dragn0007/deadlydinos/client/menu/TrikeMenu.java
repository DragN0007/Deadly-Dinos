...
public class TrikeMenu extends AbstractContainerMenu {

    public Container container;
    public Trike trike;

    public TrikeMenu(int containerId, Inventory inventory, Container container, Trike trike) {
        super(DDDMenuTypes.TRIKE_MENU.get(), containerId);
        ...
        this.addSlot(new Slot(this.container, iguaSlots++, 15, 26) {
            ...
            public boolean mayPlace(ItemStack itemStack) {
                return itemStack.is(Items.SADDLE) && !this.hasItem() && trike.isSaddleable();
            }

            @Override
            public boolean isActive() {
                return trike.isSaddleable();
            }
        });

        if(this.trike.isChested()) {
            for(int y = 0; y < 2; y++) {
                for(int x = 0; x < 7; x++) {
    ...
    public boolean stillValid(Player player) {
        return this.trike.isAlive() && this.trike.distanceTo(player) <= 8f;
    }
    ...
    public static IContainerFactory<TrikeMenu> create() {
        return (windowId, inv, data) -> {
            int containerSize = data.readInt();
            Trike igua1 = (Trike) inv.player.level.getEntity(data.readInt());
            return new TrikeMenu(windowId, inv, new SimpleContainer(containerSize), igua1);
        };
    }
}
