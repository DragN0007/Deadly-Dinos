...
public class BasiloMenu extends AbstractContainerMenu {

    public Container container;
    public Basilo para;

    public BasiloMenu(int containerId, Inventory inventory, Container container, Basilo para) {
        super(DDDMenuTypes.BASILO_MENU.get(), containerId);
    ...
    public static IContainerFactory<BasiloMenu> create() {
        return (windowId, inv, data) -> {
            int containerSize = data.readInt();
            Basilo para1 = (Basilo) inv.player.level.getEntity(data.readInt());
            return new BasiloMenu(windowId, inv, new SimpleContainer(containerSize), para1);
        };
    }
}
